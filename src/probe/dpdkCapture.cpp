...
// void DPDKCapture::replaceBlock(u_int64_t ts){
...
// void DPDKCapture::writePointerToBlock(const char* data, u_int32_t len, u_int64_t ts){
...
void DPDKCapture::writeBefore(const char* data, u_int32_t len, u_int64_t last_offset){
...
void DPDKCapture::readPacket(struct rte_mbuf *buf, u_int64_t ts, PacketMeta* meta){
    // meta->tag_num = 0;
...
u_int64_t DPDKCapture::getOffset(PacketMeta& meta){
...
void DPDKCapture::writePacketToPacketBuffer(PacketMeta& meta, u_int64_t ts, u_int64_t _offset, bool new_index){
...
FlowMetadata DPDKCapture::getFlowMetaData(PacketMeta& meta){
    // printf("pkt len:%lu\n",meta.len);
...
u_int64_t DPDKCapture::calValue(u_int64_t _offset){
    // u_int64_t value = 0;
...
u_int64_t DPDKCapture::calDiff(u_int64_t offset, u_int64_t last_offset){
    // u_int64_t diff = 0;
    // u_int64_t last_queue_id = last_offset >> 32;
...
u_int64_t DPDKCapture::calIndexNodeLen(u_int32_t key_len, u_int32_t level){
...
bool DPDKCapture::writeIndexToRing(u_int64_t value, FlowMetadata& meta, u_int64_t ts){
...
void DPDKCapture::bindCore(u_int32_t cpu){
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(cpu, &cpuset);
...
void DPDKCapture::setThreadID(u_int64_t threadID){
...
void DPDKCapture::asynchronousStop(){
    this->stop = true;
}
