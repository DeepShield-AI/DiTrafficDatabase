...
public:
    DPDKCapture(u_int32_t eth_header_len, u_int64_t disk_size, u_int64_t block_size, u_int64_t cell_size, DPDK* dpdk, PointerRingBuffer* ring,DataBlockBuffer* block_buffer, std::atomic_uint_fast64_t* diskWritePos, u_int16_t port_id, u_int16_t rx_id, u_int64_t capacity, bool bind_core = false, u_int32_t core_id = 0):
    ...
    static int launch(void *arg){
        return static_cast<DPDKCapture*>(arg)->run();
    }
};
