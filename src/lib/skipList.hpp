...
class SkipList{
    ...
    u_int32_t writeThreadCount;
    u_int32_t readThreadCount;
...
public:
    SkipList(u_int32_t maxLvl, u_int32_t keyLen, u_int32_t valueLen) :
        maxLevel(maxLvl), level(0), nodeNum(0), keyLen(keyLen), valueLen(valueLen) {
    ...
    bool insert(std::string key, u_int64_t value, u_int64_t maxNode){
        ...
        u_int32_t newLevel = randomLevel();
        ...
        // insert
        for(u_int32_t i=0; i<newLevel; ++i){
        ...
        // this->nodeNum++;
        if(this->level < newLevel) this->level = newLevel;
        return true;
    }
