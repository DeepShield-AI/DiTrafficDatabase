...
public:
    ControllerComponent(const std::string& type,const std::string& name, const std::string& logPath, const u_int64_t id) : componentType(type), componentName(name),componentID(id), logPath(logPath), running(false){
    ...
    virtual void init(ControllerContext& cfg)=0;
    virtual void run()=0;
    void start(){
